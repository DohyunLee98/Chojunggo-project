        ...
        <tr>
            <td colspan="4">
                <c:if test="${boardPage.startPage > 5}">
                    <a class="pagination-link" href="list.do?pageNo=${boardPage.startPage - 5}">[이전]</a>
                </c:if>
                <c:forEach var="pNo" begin="${boardPage.startPage}" end="${boardPage.endPage}">
                    <a class="pagination-link" id="pageNumber" href="list.do?pageNo=${pNo}&category=${param.category}">${pNo}</a>
                </c:forEach>
                <c:if test="${boardPage.endPage < boardPage.totalPages}">
                    <a class="pagination-link" href="list.do?pageNo=${boardPage.startPage + 5}">[다음]</a>
                </c:if>
            </td>
        </tr>
    </table>
